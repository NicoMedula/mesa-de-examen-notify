  ...
  static async requestPasswordReset(
    ...
    try {
      ...
      if (isDevelopment) {
        ...
        const mockToken = `test_${Date.now()}_${Math.random()
          .toString(36)
          .substring(2, 15)}`;
        ...
        console.log(
          `URL de restablecimiento: http://172.17.3.14:3000/reset-password?token=${mockToken}`
        );
        console.log("=========================");
        ...
        res.status(200).json({
          message:
            "MODO DE PRUEBA: Revisa la consola del servidor para ver el token de restablecimiento",
    ...
    try {
      ...
      if (isDevelopment) {
        ...
        if (token.startsWith("test_")) {
          ...
          console.log(
            `Nueva contraseña (simulada): ${password.substring(
              0,
              2
