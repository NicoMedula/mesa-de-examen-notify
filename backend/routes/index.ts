...
if (!VAPID_PUBLIC_KEY || !VAPID_PRIVATE_KEY) {
  console.error(
    "Faltan las claves VAPID en las variables de entorno. No se podrán enviar notificaciones push."
  );
} else {
...
router.delete(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      await PushNotificacionStrategy.getInstance().removeSubscription(
        docenteId,
        endpoint
      );
      res.json({
...
router.get(
  "/push/subscriptions/:docenteId?",
  async (req: Request, res: Response): Promise<void> => {
    const { docenteId } = req.params;

    try {
      const subscriptions =
        PushNotificacionStrategy.getInstance().getActiveSubscriptions(
          docenteId
        );
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      res.json({
        success: true,
        message: "Limpieza de suscripciones completada",
      });
    } catch (error) {
