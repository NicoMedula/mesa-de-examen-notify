...
router.get("/docente/:id/mesas", (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Error al obtener mesas', detalle: error?.message || 'Error desconocido' });
  }
});
  ...
  (async () => {
    try {
      ...
      if (error) {
        ...
        return res.status(500).json({ error: error.message, origen: 'supabase' });
      }
      
      console.log(`Encontrados ${data?.length || 0} docentes en la base de datos`);
